...
class Trader:
    ...
    def resin_orders(
        ...
        if len(order_depth.buy_orders) != 0:
            ...
            if best_bid > fair_value:
                ...
                if quantity > 0:
                    orders.append(Order("RAINFOREST_RESIN", best_bid, -abs(quantity)))
    ...
    def compute_bollinger_bands(self, prices: List[float], window: int = 20, num_std: float = 1.2) -> Tuple[float, float, float]:
        """
        计算布林带，返回 (中轨, 上轨, 下轨)
        参数默认周期20，倍率1.2
        """
        ...
        std = math.sqrt(sum((p - sma) ** 2 for p in window_prices) / window)
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            if best_bid >= fair_value + kelp_take_width:
                ...
                if bid_amount <= 20:
                    ...
                    if quantity > 0:
                        orders.append(Order("KELP", best_bid, -abs(quantity)))
                        sell_order_volume += quantity
            ...
            boll_window = 20
            num_std = 1.2  # 使用倍率1.2
            if len(self.kelp_prices) >= boll_window:
                sma, upper_band, lower_band = self.compute_bollinger_bands(self.kelp_prices, boll_window, num_std)
                logger.print("Bollinger Bands:", sma, lower_band, upper_band)
                # 如果当前中间价低于布林下轨，认为超卖，适当增加买单
                if mmmid_price < lower_band:
                    additional_buy_qty = min(5, position_limit - position - buy_order_volume)
                    if additional_buy_qty > 0:
                        orders.append(Order("KELP", best_ask - 1, additional_buy_qty))
                        buy_order_volume += additional_buy_qty
                # 如果当前中间价高于布林上轨，认为超买，适当增加卖单
                elif mmmid_price > upper_band:
                    additional_sell_qty = min(5, position_limit + position - sell_order_volume)
                    if additional_sell_qty > 0:
                        orders.append(Order("KELP", best_bid + 1, -additional_sell_qty))
                        sell_order_volume += additional_sell_qty
            # **********************************
        return orders
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # 参数设置
        resin_fair_value = 10000  # 可根据市场数据调整
        resin_width = 2
        ...
        trader_data = jsonpickle.encode({"kelp_prices": self.kelp_prices, "kelp_vwap": self.kelp_vwap})
