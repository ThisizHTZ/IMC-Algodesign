...
products = [Product.RESIN, Product.KELP, Product.INK, Product.DJEMBES, Product.JAMS, Product.CROISSANTS,
            Product.BASKET1, Product.BASKET2]
product_strings = ["RAINFOREST_RESIN", "KELP", "SQUID_INK", "DJEMBES", "JAMS", "CROISSANTS", "PICNIC_BASKET1",
                   "PICNIC_BASKET2"]
...
class Trader:
    ...
    def calculate_rpf(self, product, mid_price, position, gamma, timestamp):
        ...
        variance = statistics.variance(self.mid_prices[product]) if len(
            self.mid_prices[product]) >= 2 else 0  # variance
    ...
    def run(self, state: TradingState):
        ...
        for p in product_strings:
            if p in state.order_depths:
                mid_price = self.calculate_mid_price(state.order_depths[p].buy_orders,
        ...
        if product_strings[Product.RESIN] in state.order_depths:
            ...
            fair_price = self.calculate_fair_price(state.order_depths[product_strings[Product.RESIN]].buy_orders,
                                                   state.order_depths[product_strings[Product.RESIN]].sell_orders)

            # take best orders
            self.take_best_orders(Product.RESIN, result[product_strings[Product.RESIN]],
                                  state.order_depths[product_strings[Product.RESIN]], fair_price, 0,
                                  positions[Product.RESIN])

            # clear position
            self.clear_position_order(Product.RESIN, result[product_strings[Product.RESIN]],
                                      state.order_depths[product_strings[Product.RESIN]], fair_price,
                                      positions[Product.RESIN])

            # market make
            self.market_make(Product.RESIN, result[product_strings[Product.RESIN]],
                             state.order_depths[product_strings[Product.RESIN]], fair_price, positions[Product.RESIN],
                             self.buy_order_volume, self.sell_order_volume)

        ### KELP ###
        if product_strings[Product.KELP] in state.order_depths:
            # calculate fair price and rpf
            fair_price = self.calculate_vwap_price(state.order_depths[product_strings[Product.KELP]].buy_orders,
                                                   state.order_depths[product_strings[Product.KELP]].sell_orders)

            # clear position
            self.clear_position_order(Product.KELP, result[product_strings[Product.KELP]],
                                      state.order_depths[product_strings[Product.KELP]], fair_price,
                                      positions[Product.KELP])

            # market make
            self.market_make(Product.KELP, result[product_strings[Product.KELP]],
                             state.order_depths[product_strings[Product.KELP]], fair_price, positions[Product.KELP],
                             self.buy_order_volume, self.sell_order_volume)

        if product_strings[Product.INK] in state.order_depths:
            # calculate fair price
            fair_price = self.calculate_vwap_price(state.order_depths[product_strings[Product.INK]].buy_orders,
                                                   state.order_depths[product_strings[Product.INK]].sell_orders)

            # clear position
            self.clear_position_order(Product.INK, result[product_strings[Product.INK]],
                                      state.order_depths[product_strings[Product.INK]], fair_price,
                                      positions[Product.INK])

            # market make
            self.market_make(Product.INK, result[product_strings[Product.INK]],
                             state.order_depths[product_strings[Product.INK]], fair_price, positions[Product.INK],
                             self.buy_order_volume, self.sell_order_volume)
        ...
        if (
            ...
            synthetic_basket1_price = 6 * mid_prices[Product.CROISSANTS] + 3 * mid_prices[Product.JAMS] + mid_prices[
                Product.DJEMBES]
            spread = mid_prices[Product.BASKET1] - synthetic_basket1_price
            logger.print(
                f"Basket1 Price: {mid_prices[Product.BASKET1]}, Synthetic Basket1 Price: {synthetic_basket1_price}, Spread: {spread}")
            ...
            if spread > threshold:
                # sell basket1 at bid
                self.safe_order(Product.BASKET1, best_bids[Product.BASKET1], -1, positions[Product.BASKET1],
                                result[product_strings[Product.BASKET1]])

                # buy components at ask
                self.safe_order(Product.CROISSANTS, best_asks[Product.CROISSANTS], 6, positions[Product.CROISSANTS],
                                result[product_strings[Product.CROISSANTS]])
                self.safe_order(Product.JAMS, best_asks[Product.JAMS], 3, positions[Product.JAMS],
                                result[product_strings[Product.JAMS]])
                self.safe_order(Product.DJEMBES, best_asks[Product.DJEMBES], 1, positions[Product.DJEMBES],
                                result[product_strings[Product.DJEMBES]])

            # -spread -> basket1 is underpriced
            elif spread < -threshold:
                # buy basket1 at ask
                self.safe_order(Product.BASKET1, best_asks[Product.BASKET1], 1, positions[Product.BASKET1],
                                result[product_strings[Product.BASKET1]])

                # sell components at bid
                self.safe_order(Product.CROISSANTS, best_bids[Product.CROISSANTS], -6, positions[Product.CROISSANTS],
                                result[product_strings[Product.CROISSANTS]])
                self.safe_order(Product.JAMS, best_bids[Product.JAMS], -3, positions[Product.JAMS],
                                result[product_strings[Product.JAMS]])
                self.safe_order(Product.DJEMBES, best_bids[Product.DJEMBES], -1, positions[Product.DJEMBES],
                                result[product_strings[Product.DJEMBES]])
