...
class Trader:
    ...
    def compute_orders_basket(self, order_depth):
        ...
        res_buy = mid_price['PICNIC_BASKET'] - mid_price['DIP'] * 4 - mid_price['BAGUETTE'] * 2 - mid_price[
            'UKULELE'] - 375
        res_sell = mid_price['PICNIC_BASKET'] - mid_price['DIP'] * 4 - mid_price['BAGUETTE'] * 2 - mid_price[
            'UKULELE'] - 375
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        _, bs_bananas = self.values_extract(
            collections.OrderedDict(sorted(state.order_depths['BANANAS'].sell_orders.items())))
        _, bb_bananas = self.values_extract(
            collections.OrderedDict(sorted(state.order_depths['BANANAS'].buy_orders.items(), reverse=True)), 1)
