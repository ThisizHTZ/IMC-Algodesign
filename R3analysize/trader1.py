...
class Trader:
    ...
    def get_best_bid(self, order_depth, prod):

        if prod in order_depth.keys():
            best_bid_price = max(order_depth[prod].buy_orders.keys())
            best_bid_vol = abs(order_depth[prod].buy_orders[best_bid_price])
        else:
            best_bid_price = None
            best_bid_vol = None
    ...
    def get_best_ask(self, order_depth, prod):

        if prod in order_depth.keys():
            best_ask_price = min(order_depth[prod].buy_orders.keys())
            best_ask_vol = abs(order_depth[prod].buy_orders[best_ask_price])
        else:
            best_ask_price = None
            best_ask_vol = None
    ...
    def construct_artificial_basket1_order_depth(
        ...
        croissants_best_bid = max(order_depths[Product.CROISSANTS].buy_orders.keys()) if order_depths[
            Product.CROISSANTS].buy_orders else 0
        croissants_best_ask = min(order_depths[Product.CROISSANTS].sell_orders.keys()) if order_depths[
            Product.CROISSANTS].sell_orders else float('inf')

        jams_best_bid = max(order_depths[Product.JAMS].buy_orders.keys()) if order_depths[
            Product.JAMS].buy_orders else 0
        jams_best_ask = min(order_depths[Product.JAMS].sell_orders.keys()) if order_depths[
            Product.JAMS].sell_orders else float('inf')

        djembes_best_bid = max(order_depths[Product.DJEMBES].buy_orders.keys()) if order_depths[
            Product.DJEMBES].buy_orders else 0
        djembes_best_ask = min(order_depths[Product.DJEMBES].sell_orders.keys()) if order_depths[
            Product.DJEMBES].sell_orders else float('inf')

        # Get implied bid and ask for the basket
        if (croissants_best_bid != 0) & (jams_best_bid != 0) & (
                djembes_best_bid != 0):  # If you don't have at least one bid for each of the products you can't make the basket
        ...
        if artificial_basket1_implied_bid > 0:
            croissants_bid_volume = order_depths[Product.CROISSANTS].buy_orders[
                                        croissants_best_bid] // croissants_weight
        ...
        if artificial_basket1_implied_ask < float('inf'):
            croissants_ask_volume = order_depths[Product.CROISSANTS].buy_orders[
                                        croissants_best_ask] // croissants_weight
    ...
    def construct_artificial_basket_order_depth(  # generalized version of the previous function
        ...
        basket_prods = self.baskets_products[basket]
        ...
        for product, weight in basket_prods.items():
            ...
            best_ask = min(order_depths[product].sell_orders.keys()) if order_depths[product].sell_orders else float(
                'inf')
    ...
    def convert_artificial_basket_orders(self,
        ...
        basket_composition = self.baskets_products[basket]
        ...
        artificial_order_depth = self.construct_artificial_basket_order_depth(order_depths, basket)
        ...
        best_ask = min(artificial_order_depth.sell_orders.keys()) if artificial_order_depth.sell_orders else float(
            'inf')
    ...
    def spread_orders(self,
        ...
        window = self.params[Product.SPREAD1]["spread_std_window"]
        ...
        spread_mean = (np.sum(spread_history) + (
                    self.params[Product.SPREAD1]["spread_mean"] * self.params[Product.SPREAD1]["starting_its"])) \
                      / (self.params[Product.SPREAD1]["starting_its"] + len(spread_history))
        spread_std = np.std(spread_history[-window:])
        zscore = (spread - spread_mean) / spread_std

        # Decide on action based on the z-score thresholds.
        if zscore >= 50:  # self.params[Product.SPREAD1]["zscore_threshold"]:
            ...
            if basket_position == -self.params[Product.SPREAD1]["target_position"]:
                return None
            target_quantity = abs(-self.params[Product.SPREAD1]["target_position"] - basket_position)
            ...
            aggregate_orders = self.convert_artificial_basket_orders(artificial_orders, order_depths, basket)
            aggregate_orders[basket] = basket_orders
            return aggregate_orders

        if zscore <= -50:  # -self.params[basket]["zscore_threshold"]:
    ...
    #     # Build the artificial basket order depth from component order books.
    #     artificial_order_depth = self.construct_artificial_basket_order_depth(order_depths, basket_product)

    #     underlying_prods_weights = {}

    #     underlying_prods_best_bid_price = {}
    #     underlying_prods_best_ask_price = {}
    #     underlying_prods_best_bid_vol = {}
    #     underlying_prods_best_ask_vol = {}

    #     for prod, weight in self.baskets_products[basket_product].items():

    #         underlying_prods_weights[prod] = weight

    #         best_bid_price, bes_bid_vol = self.get_best_bid(order_depths, prod)
    #         best_ask_price, bes_ask_vol = self.get_best_ask(order_depths, prod)

    #         underlying_prods_best_bid_price[prod] = best_bid_price
    #         underlying_prods_best_ask_price[prod] = best_ask_price

    #         underlying_prods_best_bid_vol[prod] = bes_bid_vol
    #         underlying_prods_best_ask_vol[prod] = bes_ask_vol

    #     # We check if we have a bid and ask price for every product
    #     if all(value is not None for value in underlying_prods_best_bid_price.values()) and all(value is not None for value in underlying_prods_best_ask_price.values()):
    ...
    def run(self, state: TradingState):
        ...
        spread_orders = self.spread_orders(state.order_depths, Product.PICNIC_BASKET1, basket_position,
