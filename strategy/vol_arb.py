...
class BlackScholes:
    ...
    def black_scholes_call(spot, strike, time_to_expiry, volatility):
        d1 = (math.log(spot) - math.log(strike) + (0.5 * volatility * volatility) * time_to_expiry) / (
...
class Trader:
    ...
    def vol_spread_arb(self, threshold, option, result, option_order_depth, spot_order_depth, mid_prices, spot_pos,
                       option_pos, timestamp):
        ...
        if spread >= threshold:
            if len(option_order_depth.buy_orders) > 0:
                ...
                if len(spot_order_depth.sell_orders) > 0:
                    ...
                    if hedge_qty > 0:
                        result[product_strings[Product.VOLCANIC_ROCK]].append(
                            Order(product_strings[Product.VOLCANIC_ROCK], best_ask, hedge_qty))
        ...
        # long voucher
            ...
            if len(option_order_depth.sell_orders) > 0:
                ...
                if len(spot_order_depth.buy_orders) > 0:
                    ...
                    if hedge_qty > 0:
                        result[product_strings[Product.VOLCANIC_ROCK]].append(
                            Order(product_strings[Product.VOLCANIC_ROCK], best_bid, -hedge_qty))
    ...
    def delta_hedge(self, voucher: Product, result, volcano_order_depth, volcano_position, voucher_position, delta):
        if result[product_strings[voucher]] == None or len(result[product_strings[voucher]]) == 0:
            ...
            future_voucher_position = voucher_position + sum(
                order.quantity for order in result[product_strings[voucher]])
    ...
    def run(self, state: TradingState):
        ...
        for p in product_strings:
            if p in state.order_depths:
                ...
                best_ask = min(state.order_depths[p].sell_orders.keys()) if state.order_depths[
                    p].sell_orders else float("inf")
            else:
        ...
        self.vol_spread_arb(0.005, p, result, state.order_depths[product_strings[p]],
                            state.order_depths[product_strings[Product.VOLCANIC_ROCK]], mid_prices,
                            positions[Product.VOLCANIC_ROCK], positions[p], state.timestamp)
